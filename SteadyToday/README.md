...
- **오늘의 도전** 섹션: 가로 스크롤 카드 형태로 표시
  - 시작 전: 00:00:00 표시, 시작 버튼
  - 진행 중: 실시간 타이머 표시, 종료 버튼 (어제 시작한 진행 중인 세션도 오늘 표시)
  - 완료: 완료 시간 + 목표 시간 표시, 성공/종료 배지 (시작 날짜 기준으로 집계)
- **오늘의 액션** 섹션: 데일리(요일 반복) 액션들, 오늘 요일에 해당하는 액션만 노출
- **드래그 정렬**: Edit 모드에서 액션 순서 변경 가능(전역 `todayOrder` 저장)
...
- 액션 생성 및 편집(주 N회 / 요일 반복 / 오늘의 도전)
  - 오늘의 도전 액션: 목표 시간(시간/분)만 설정, 요일 반복은 자동으로 월~일 전체로 설정
...
- Today 화면의 Edit 모드는 구현되어 있으나, 현재는 시스템 Edit 모드에 의존(추후 Edit 버튼 추가 가능).
